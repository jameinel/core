...
GOPATH=$WORK go get -v -d github.com/wallyworld/core/...

echo "Setting juju-core tree to $TAG."
(cd "${WORK}/src/github.com/wallyworld/core/" && bzr revert -r $TAG)

echo "Updating juju-core dependencies to the required versions."
GOPATH=$WORK godeps -u "${WORK}/src/github.com/wallyworld/core/dependencies.tsv"

# Smoke test
GOPATH=$WORK go build -v github.com/wallyworld/core/...

# Change the generic release to the proper juju-core version.
VERSION=$(sed -n 's/^const version = "\(.*\)"/\1/p' \
    $WORK/src/github.com/wallyworld/core/version/version.go)
